...
public final class ApiClient {
  ...
  public static UpdatePostResponse check(String pack, UpdatePostRequest manifest, Path instance)
    ...
    for (String server : Config.getServers(instance)) {
      try {
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create(server + "/api/v3/update/" + encodedPack))
  ...
  public static UpdateGetResponse getScanPaths(String pack, Path instance)
      throws IOException, InterruptedException {
    ...
    for (String server : Config.getServers(instance)) {
      try {
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create(server + "/api/v2/update/" + encodedPack))
            .timeout(Duration.ofSeconds(10))
            .GET()
            .build();
